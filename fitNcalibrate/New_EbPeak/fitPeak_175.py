...
def plotter(h=None,name=None):
    ...
    gROOT.Reset()
    h.UseCurrentStyle()
    fitfunc_gaus = TF1("Gaussian", myFitFunc, 65.0, 70.0, 3)
    ## Set normalization
...
def plotter_cal(h=None,name=None):
    ...
    gROOT.Reset()
    h.UseCurrentStyle()
    fitfunc_gaus = TF1("Gaussian", myFitFunc, 67.0, 72.0, 3)
    ## Set normalization
    fitfunc_gaus.SetParameter(0, h.Integral());
    fitfunc_gaus.SetParLimits(0, 0.1*h.Integral(), 2.0*h.Integral());
    ## Set gaussian mean starting value and limits
    fitfunc_gaus.SetParameter(1, 69.5);
    fitfunc_gaus.SetParLimits(1, 67.0, 72.0);
    ## Set gaussian width starting value and limits
    fitfunc_gaus.SetParameter(2, 0.05);
    fitfunc_gaus.SetParLimits(2, 0.1, 2.0);

    h.Fit("Gaussian","EM","",67.0,72.0)
    h.Draw()
...
def main():
    ...
    E_peak = TH1F("E_peak", "", 100,64,74) # 175v5
...
#    plotter(E_peak,"Eb_m175p5.pdf")
#    plotter_cal(Eb_calibrated,"Eb_cal_m175p5.pdf")

#    plotter_err(E_peak_err,"Eb__m175p5_err.pdf")
#    plotter_cal_err(delEb_calibrated,"delEb_m175p5.pdf")



#   plotter_err(hpull,"Pull_m172p5.pdf")
    plotter_err(hpullcal,"Pull_corr_m175p5.pdf")
